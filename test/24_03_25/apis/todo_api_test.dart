...
void main() {
  group('TodoApi 클래스', () {
    ...
    group('getTodo 메소드는', () {
      test(
          'https://jsonplaceholder.typicode.com/todos/1 엔드포인트에서 받은 json을 Todo 단일객체로 매핑하여 반환한다.',
          () async {
        // Given
        const userId = 1;
        const id = 1;
    ...
    group('getTodos 메소드는', () {
      test(
          'https://jsonplaceholder.typicode.com/todos 엔드포인트에서 받은 json을 List<Todo>로 매핑하여 반환한다.',
          () async {
        // Given
        const userId = 10;
        const id = 200;
        const title = 'ipsam aperiam voluptates qui';
        const completed = false;
        Todo expected =
            Todo(userId: userId, id: id, title: title, completed: completed);

        // When
        List<Todo> result = await todoApi.getTodos();
