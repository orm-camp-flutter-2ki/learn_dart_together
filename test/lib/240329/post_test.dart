...
import 'package:learn_dart_together/240329/repository/post_repo_impl.dart';
import 'package:learn_dart_together/240329/repository/post_repository.dart';
import 'package:test/expect.dart';
import 'package:test/scaffolding.dart';

import '../240327/bank_test.dart';

void main() {
  group('(선택) 연습문제 6. RestAPI 및 CRUD 연습', () {
    final PostRepository repository = PostRepositoryImpl();

    test('Future<Post> getPost(int id)', () async {
      // Given
      final int id = 2;
      final mockJsonData = PostDto.fromJson(postMockData);

      // When
      final getPost = await repository.getPost(id);

      // Then
      expect(getPost.userId, mockJsonData.userId);
      expect(getPost.id, mockJsonData.id);
      expect(getPost.title, mockJsonData.title);
      expect(getPost.body, mockJsonData.body);
    });

    test('Future<List<Post>> getPostsAll()', () async {
      // When
      final getAll = await repository.getPostsAll();

      // Then
      expect(getAll.length, postMockWholeData.length);
      expect(
        Post(
          userId: getAll.first.userId,
          id: getAll.first.id,
          title: getAll.first.title,
          body: getAll.first.body,
        ),
        Post(
            userId: postMockWholeData[0]['userId'],
            id: postMockWholeData[0]['id'],
            title: postMockWholeData[0]['title'],
            body: postMockWholeData[0]['body']),
      );

      expect(
        Post(
          userId: getAll.last.userId,
          id: getAll.last.id,
          title: getAll.last.title,
          body: getAll.last.body,
        ),
        Post(
            userId: postMockWholeData[postMockWholeData.length - 1]['userId'],
            id: postMockWholeData[postMockWholeData.length - 1]['id'],
            title: postMockWholeData[postMockWholeData.length - 1]['title'],
            body: postMockWholeData[postMockWholeData.length - 1]['body']),
      );
    });

    test(
        'Future<List<Post>> getPosts({int page, int limit}) limit 갯수만큼 나눴을 때 page 번째만큼',
        () async {
          // Given
          final page = 1;
          final limit = 2;
          ...
          // When
          final pages = await repository.getPosts(page: page, limit: limit);

          // Then
          expect(pages.length, expectedLength);
        });

    test('Future<int> addPost(PostDto postDto) 추가된 Post의 id를 리턴', () async {
      // Given
      final getAll = await repository.getPostsAll();
      final dataList = getAll.length;
      final expected = 101;

      final newData = PostDto(
        userId: dataList + 1,
        id: dataList + 1,
        title: 'new title',
        body: 'new body',
      ...
      // When
      final addResult = await repository.addPost(newData);

      // Then
      expect(addResult, expected);
    });

    test('Future<Post> updatePost(PostDto postDto) 변경된 객체를 리턴', () async {
      // Given
      final id = 2;
      final newData = PostDto(userId: 1, id: 2, title: 'changedTitle', body: 'updatedBody');
      final originalData = PostDto.fromJson(postMockData);

      // When
      final updatedData = await repository.updatePost(id, newData);

      // Then
      expect(originalData.id, updatedData.id);
      expect(updatedData.title, 'changedTitle');
    });

    test('Future<bool> deletePost(int postId) 성공 여부를 리턴', () async {
      // Given
      final id = 2;

      // When
      final isDeleted = await repository.deletePost(id);
...
final Map<String, dynamic> postMockData = {
  "userId": 1,
  "id": 2,
  "title": "qui est esse",
  "body":
...
final List<Map<String, dynamic>> postMockWholeData = [
  {
    "userId": 1,
    "id": 1,
    "title":
