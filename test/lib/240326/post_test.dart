...
import 'package:learn_dart_together/240326/respository_impl/post_repo_impl.dart';
import 'package:test/expect.dart';
import 'package:test/scaffolding.dart';

void main() {
  group('(선택) 과제 6. Repository 작성 연습 6', () {
    PostRepository repository = PostRepositoryImpl();

    test('Future<Post> getPost(int id) 생성', () async {
      // Given
      final id = 2;
      final expectedTitle = 'qui est esse';

      // When
      final getPostData = await repository.getPost(id);

      // Then
      expect(getPostData.title, equals(expectedTitle));
    });

    test('Future<List<Post>> getPosts() 생성', () async {
      // Given
      final expected = expectedPosts.map(Post.fromJson).toList();
    ...
    test('Future<List<Post>> getPosts(page: 1, limit: 3) 생성', () async {
      ...
      final expected = expectedPages.map(Post.fromJson).toList();

      // When
      final page = 1;
      final limit = 3;
      final pages = await repository.getPostPages(page, limit);

      expect(pages.equals(expected), true);
    });
  });
}
