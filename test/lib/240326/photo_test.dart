...
import 'package:learn_dart_together/240326/respository_impl/photo_repo_impl.dart';
import 'package:test/scaffolding.dart';
import 'package:test/test.dart';

void main() async {
  PhotoRepositoryImpl repositoryImpl = PhotoRepositoryImpl();
  
  group('과제 3. Repository 작성 연습 3', () {
    test('Future<List<Photo>> getPhotos(int albumId) 생성', () async {
      // Given
      final albumId = 1;

      // When
      final data = await repositoryImpl.getPhotos(albumId);
      final expected = expectedPhotos.map(Photo.fromJson).toList();

      // Then
      expect(data.equals(expected), true);
    });
  });
