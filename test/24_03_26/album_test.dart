...
import 'package:learn_dart_together/24_03_26/repository/album_repo_impl.dart';
...
void main() {
  group('about album', () {
    test('getAlbums test', () async {
      List<Album> albums = await AlbumRepositoryImpl().getAlbums();

      final expected = expectedAlbums.map(Album.fromJson).toList();

      expect(albums.equals(expected), true);
    });

    test('getAlbumsTop10 test', () async {
      List<Album> albumsTop10 = await AlbumRepositoryImpl().getAlbumsTop10();

      final expected = exptectedAlbumsTop10.map(Album.fromJson).toList();

      expect(albumsTop10.equals(expected), true);
    });
  });
}

final exptectedAlbumsTop10 = [
