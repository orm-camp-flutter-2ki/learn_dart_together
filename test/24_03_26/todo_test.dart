...
void main() {
  group('about Todo', () {
    test('getTodos test', () async {
      List<Todo> albums = await TodoRepositoryImpl().getTodos();
    ...
    test('getCompletedTodos test', () async {
      // to에서 completed가 true인 것들
      List<Todo> completedTodo = await TodoRepositoryImpl().getCompletedTodos();

      bool isTrue = true;

      for (final completedTodoEl in completedTodo) {
        // 만약에 false가 하나라도 있다면
        if (!completedTodoEl.completed) {
          isTrue = false;

          // isTrue는 false가 되고 break
          break;
        }
      }

      // expect에서 isTrue가 false인지 검사
      // false가 있다는 것은 completedTodd가 잘못 되었다는 뜻이니까
