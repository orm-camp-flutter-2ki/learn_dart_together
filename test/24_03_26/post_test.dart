...
import 'package:learn_dart_together/24_03_26/repository/post_repo_imple.dart';
import 'package:test/expect.dart';
import 'package:test/scaffolding.dart';

void main() {
  group('about post', () {
    test('get post test', () async {
      Post postIdOne = await PostRepositoryImpl().getPost(1);

      expect(postIdOne.id == 1, true);
    });

    test('get posts test', () async {
      List<Post> pagenated = await PostRepositoryImpl().getPosts(1, 3);

      final expected = expectedPa.map(Post.fromJson).toList();

      expect(pagenated.equals(expected), true);
    });

    test('get posts all test', () async {
      List<Post> postsAll = await PostRepositoryImpl().getPostsAll();

      final expected = expectedAll.map(Post.fromJson).toList();

      expect(postsAll.equals(expected), true);
    });
  });
}
