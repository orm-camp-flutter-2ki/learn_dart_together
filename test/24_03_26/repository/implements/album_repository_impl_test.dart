...
import 'package:learn_dart_together/24_03_26/repository/implements/album_repository_impl.dart';
import 'package:learn_dart_together/24_03_26/repository/interface/album_repository.dart';
import 'package:test/test.dart';

void main() {
  group('Album Repository 메서드 테스트', () {
    final AlbumRepository repo = AlbumRepositoryImpl();

    test('getAlbums 메서드 테스트', () async {
      final List<Album> albumList = await repo.getAlbums();
      final given = albumList.map((e) => e.toJson()).toList();
      final expectValue = [
        {
          "userId": 1,
          "id": 1,
      ...
      expect(DeepCollectionEquality().equals(given, expectValue), true);
    });

    test('getAlbumTop10 메서드 테스트', () async {
      final List<Album> albumList = await repo.getAlbumsTop10();
      final given = albumList.map((e) => e.toJson()).toList();
      final expectValue = [
        {
          "userId": 1,
