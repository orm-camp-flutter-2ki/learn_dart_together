...
void main() {
  group('movie test', () {
    test('movie Info', () async {
      final MovieInfoRepository repository = MovieInfoRepositoryImpl();
      final getMovieInfo = await repository.getMovieInfoList();
      final expectMovieInfo =
          movieInfo.map((movieInfo) => MovieInfo.fromJson(movieInfo)).toList();
      expect(getMovieInfo.equals(expectMovieInfo), true);
    });

    test('movie detail', () async {
      final MovieRepository repository = MovieRepositoryImpl();

      final getMovieDetail = await repository.getMovie(1011985);
      final expectMovieDetail = MovieInfo.fromJson(movieDetail);

      expect(getMovieDetail.adult, expectMovieDetail.adult);
    });
  });
}
