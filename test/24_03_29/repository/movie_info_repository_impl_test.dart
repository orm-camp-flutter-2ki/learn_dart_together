...
void main() {
  group('MovieInfoRepository test', () {
    final MovieInfoApi movieInfoApi = MovieInfoApi();
    final MovieInfoRepository repo = MovieInfoRepositoryImpl(api: movieInfoApi);

    test('getMovieInfoList method test',() async {
      final List<MovieInfo> moveInfoList = await repo.getMovieInfoList();

      final expectValue = "쿵푸팬더 4";
      final given = moveInfoList[0].title;

      expect(given, expectValue);
    });

    test('getMovieInfoList length compare with raw data',() async {
      final List<MovieInfo> moveInfoList = await repo.getMovieInfoList();

      final expectValue = jsonData.length;
      final given = moveInfoList.length;

      expect(given, expectValue);
    });

    test('getMovie method test',() async {
      //쿵푸 팬더
      final MovieInfoDetail moveInfo = await repo.getMovie(1011985);

      final expectValue = "마침내 내면의 평화… 냉면의 평화…가 찾아왔다고 믿는 용의 전사 ‘포’ 이젠 평화의 계곡의 영적 지도자가 되고, 자신을 대신할 후계자를 찾아야만 한다. “이제 용의 전사는 그만둬야 해요?” 용의 전사로의 모습이 익숙해지고 새로운 성장을 하기보다 지금 이대로가 좋은 ‘포’ 하지만 모든 쿵푸 마스터들의 능력을 그대로 복제하는 강력한 빌런 ‘카멜레온’이 나타나고 그녀를 막기 위해 정체를 알 수 없는 쿵푸 고수 ‘젠’과 함께 모험을 떠나게 되는데… 포는 가장 강력한 빌런과 자기 자신마저 뛰어넘고 진정한 변화를 할 수 있을까?";
      final given = moveInfo.overview;
    ...
    test('getMovie method test',() async {
      //마담 웹
      final MovieInfoDetail moveInfo = await repo.getMovie(634492);

      final expectValue = 2201.481;
      final given = moveInfo.popularity;
...
final List<Map<String,dynamic>> jsonData = [
  ...
  },
  {
    ...
    "overview": "혁명적인 음악으로 사랑과 화합의 메시지를 전한 시대의 아이콘 ‘밥 말리’의 전설적인 무대와 나아가 세상을 바꾼 그의 뜨거웠던 삶을 그린 감동 음악 영화",
    "popularity": 402.513,
  ...
  },
  {
    ...
    "overview": "",
    "popularity": 334.567,
    "poster_path": "/vlRhgfrfBzJKjeGwoUjTvSwk8gi.jpg",
