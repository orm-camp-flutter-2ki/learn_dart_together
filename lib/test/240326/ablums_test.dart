...
import '../../240326/model/album.dart';

void main() {
  test('albums Test', () async {
    final api = AlbumApi();
    final albums = await api.getAlbums();
    final expected = expectedList.map((Album.fromJson)).toList();
    expect(albums.equals(expected), true);
  });

  test('album top10 Test', () async {
    final repository = AlbumRepositoryImpl();
    final List<Album> top10 = await repository.getAlbumsTop10();
    print(top10);
  });
}
